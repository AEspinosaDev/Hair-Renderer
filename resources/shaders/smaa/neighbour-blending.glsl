...
void SMAANeighborhoodBlendingVS(vec2 texcoord,
    ...
    offset = fma(v_rt_metrics.xyxy, vec4( 1.0, 0.0, 0.0,  1.0), texcoord.xyxy);
}
...
float4 SMAANeighborhoodBlendingPS(float2 texcoord,
    ...
    if (dot(a, float4(1.0, 1.0, 1.0, 1.0)) < 1e-5) {
        ...
        float4 blendingCoord = mad(blendingOffset, float4(v_rt_metrics.xy, -v_rt_metrics.xy), texcoord.xyxy);
